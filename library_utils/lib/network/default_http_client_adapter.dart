...
class DefaultHttpClientAdapter extends HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    if (options.connectTimeout > 0) {
      requestFuture = requestFuture.timeout(Duration(milliseconds: options.connectTimeout));
    }
    ...
    return ResponseBody(
      ...
      redirects:
          responseStream.redirects.map((e) => RedirectRecord(e.statusCode, e.method, e.location)).toList(),
  ...
  HttpClient _configHttpClient(Future cancelFuture, int connectionTimeout) {
    ...
    if (cancelFuture != null) {
      ...
      _defaultHttpClient.idleTimeout = Duration(seconds: 30);
      if (onHttpClientCreate != null) {
        //user can return a HttpClient instance
